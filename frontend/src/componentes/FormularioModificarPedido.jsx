...
import './FormularioModificarPedido.css';
import {useState,useEffect} from 'react';

function FormularioModificarPedido({pedidoAEditar, cerrarFormularioModificar, onSubmit, mostrarAlerta, mostrarResultadoOperacion}) {
        ...
        const nuevoPedido = {
          ...
          try{
            ...
            if (response.ok){
                ...
                onSubmit(data);
                mostrarResultadoOperacion("✅ Pedido modificado.");
            }else{
    ...
    useEffect(() => {
        const traerOpciones = async () => {
          try {
            ...
            if (response.ok) {
              const pedidoB2 = await response.json();
              setOpciones(pedidoB2);
            } else {
      ...
      useEffect(() => {
        if (pedidoAEditar) {
            const pedidoElemento = pedidoAEditar[0];
          setPedido({
            nombreCliente:pedidoElemento.nombreCliente,
            emailCliente:pedidoElemento.emailCliente,
            telefonoCliente:pedidoElemento.telefonoCliente,
            direccionCliente:pedidoElemento.direccionCliente,
            hamburguesas: pedidoElemento.hamburguesa,
            papas: pedidoElemento.papas,
            bebidas: pedidoElemento.bebida,
            id: pedidoElemento.id
          });
        }
      }, [pedidoAEditar]);
    ...
    return (
      ...
      <div className='popup-formulario-pedido'>
        ...
        <div className='contenedor-botones'>
          <button type="submit" className="boton-pedido" onClick={actualizarPedido}>Enviar</button>
          <button type="button" className="boton-pedido-cancelar" onClick={cerrarFormularioModificar}>Cancelar</button>
        </div>
      </div>
    </div>
    );      
...
export default FormularioModificarPedido    
